...
## agent_communication:
##     -agent: "main"
##     -message: "VOICE AGENT SPEECH RECOGNITION INFINITE LOOP ISSUE FIXED: User reported critical issue with AI Voice Agent where clicking the voice agent button caused infinite loop of 'Speech recognition started' -> 'Speech recognition error: aborted' -> 'Speech recognition ended' repeating continuously. Issue was caused by auto-restart mechanism in onend handler that immediately tried to restart recognition after it was aborted, creating an infinite loop. COMPREHENSIVE FIXES IMPLEMENTED: 1) Enhanced state management system with recognitionState tracking to prevent race conditions, 2) Added retry limits (max 5) with exponential backoff delays, 3) Categorized speech recognition errors into fatal/retryable/unknown with appropriate handling, 4) Improved cleanup logic with proper timeout management, 5) Added debouncing for restart attempts (1.5s delay), 6) Enhanced error handling for different error types, 7) Better auto-listen toggle with proper state validation, 8) Added initialization state tracking, 9) Improved UI with detailed state indicators, 10) Added retry counter display for user feedback. The Voice Agent now requires comprehensive frontend testing to verify that the infinite loop issue is resolved and that all voice functionality works correctly including microphone permission handling, speech recognition accuracy, speech synthesis, conversation flow, error recovery, and user interface responsiveness."
##     -agent: "testing"
